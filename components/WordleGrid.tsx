...
export const WordleGrid: React.FC<WordleGridProps> = ({ 
  ...
  return (
    <div className={`flex flex-col gap-2.5 sm:gap-3 stable-transform w-full ${isMessageMode ? 'items-center' : ''}`}>
      {rows.map((_, i) => (
        <Row 
          key={i} 
...
const Row: React.FC<{ 
  ...
  return (
    <div className="flex gap-2.5 sm:gap-3 justify-center w-full">
      {cells.map((_, i) => {
...
const Cell: React.FC<{ char: string; status: LetterStatus; isMessageMode?: boolean }> = ({ char, status, isMessageMode }) => {
  ...
  if (isMessageMode) {
    if (char && char !== ' ') {
      style = "bg-rose-500 border-rose-600 text-white shadow-lg";
    } else {
    ...
    style = "bg-slate-50 border-slate-200 text-slate-400 grayscale-[0.5]";
  } else if (char) {
    style = "bg-white border-rose-300 text-rose-900 animate-pop scale-105";
  }

  return (
    <div className={`w-12 h-12 sm:w-14 sm:h-14 border-2 rounded-xl flex items-center justify-center font-bold text-xl sm:text-2xl transition-all duration-500 transform-gpu ${style} stable-transform flex-shrink-0`}>
      {char}
    </div>
  );
};
